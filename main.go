...
func main() {
	host := flag.String("host", "0.0.0.0", "Host/IP to bind (use 0.0.0.0 for all interfaces)")
	port := flag.Int("port", 43210, "Port to listen on")
	token := flag.String("token", os.Getenv("JSON_SERVER_API_TOKEN"), "Bearer token required in the Authorization header (also via JSON_SERVER_API_TOKEN)")
	jsonDir := flag.String("json-dir", "json", "Directory containing JSON files")
	verbose := flag.Bool("verbose", false, "Enable request logging to stdout")
	logFile := flag.String("log-file", "", "Optional path to write request logs")
	flag.Parse()

	if *token == "" {
		log.Fatal("API token is required (set -token or JSON_SERVER_API_TOKEN)")
	}
	...
	if len(logOutputs) > 0 {
		requestLogger = log.New(io.MultiWriter(logOutputs...), "", log.LstdFlags)
	}
	...
	log.Printf("Starting Go json-server on %s using %s", addr, *jsonDir)
...
func (s *server) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	if s.logger != nil {
		ip := clientIP(r)
		s.logger.Printf("%s %s %s %d %s", ip, r.Method, r.URL.Path, lrw.status, time.Since(start).Round(time.Millisecond))
	}
}
...
func filterItems(items []interface{}, q url.Values) []interface{} {
	...
	if sortField != "" {
		...
		sort.SliceStable(filtered, func(i, j int) bool {
			...
			lv, _ := nestedValue(left, sortField)
			rv, _ := nestedValue(right, sortField)
			return compareValues(lv, rv, desc)
		})
	}
...
func matchQuery(obj map[string]interface{}, q url.Values) bool {
	for key, vals := range q {
		...
		value, ok := nestedValue(obj, key)
		if !ok {
			return false
		}
		hay := normalize(value)
		if strings.ContainsAny(needle, "*?") {
			if !wildcardMatch(hay, needle) {
				return false
			}
			continue
		}
...
// nestedValue walks dot-separated keys (e.g., "author.name") inside a map.
func nestedValue(obj map[string]interface{}, path string) (interface{}, bool) {
	if obj == nil {
		return nil, false
	}
